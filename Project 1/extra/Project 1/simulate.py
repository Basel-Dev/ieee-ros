...
def drawPath(path, placeMarker=True):
    xPoints = [x for [x, y] in path.pathList]
    yPoints = [y for [x, y] in path.pathList]

    (goalX, goalY) = path.goal
    ax.plot(xPoints, yPoints)
    if placeMarker:
        ax.plot([goalX], [goalY], "g^", zorder=9)


def drawObstacleGrid(obstacles):
...
drawObstacleGrid(pathFinding.getObstacles())

def interpolatePath(drone, state, package, path, subdivisionsPerPoint):
    denseSampleArray = []
    interpolationArray = np.linspace(0, 1, subdivisionsPerPoint)

    for step, point in enumerate(path.pathList):
        if step+1 < len(path.pathList):
            npPoint = np.array(point)
            npNextPoint = np.array(path.pathList[step+1])
...
def getFinalInterpolatedSequence(drone, package, path, subdivisionsPerPoint, totalTime, pauseTime):
    ...
    returnBackArray = interpolatePath(drone, DroneStatus.RETURNING, package, path.getReversePath(), subdivisionsPerPoint)
...
def executePathwalk(drone, package, path, totalTime, pauseTime):
    drawPath(path)
    drawPath(path.getReversePath(), False)
    subdivisions = 10
...
def simulateDrone(drone, package, start, goal):
    uiDrone = UIDrone(drone.id, drone.max_weight)
    uiPath = Path(start, goal)
    anim = executePathwalk(uiDrone, package, uiPath, TIME_RUNNING, PAUSE_TIME)
    plt.show()
